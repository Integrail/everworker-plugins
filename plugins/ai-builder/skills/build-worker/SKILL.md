...
6. **Ask permission to test, then test.** Execute the worker with a representative `userMessage` via `mcp__ai_builder__worker_execute` (pass plain `userMessage: "..."` — the tool wraps it as `IChatMessage` server-side). Do **not** use `workflow_execute` for workers; it expects `userMessage` as an object and will produce `content: null` errors with a plain string. Poll `workflow_execution_status` using the same loop pattern as `/ai-builder:test` (echo executionId before first poll, 5s gaps via `Bash sleep 5`, cap 30 iterations, 3-strikes retry on transient errors). Inspect:
