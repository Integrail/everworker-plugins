...
- **Compute `studioData.nodes[].position` for every workflow before deploy — see PLAYBOOK § NODE LAYOUT.** New nodes get topo-ordered placement with fan spacing; existing positions in the local file are carried through verbatim so canvas-side drags are never overwritten.
